...
public interface AnnouncementRepository extends JpaRepository<Announcement, Long> {
    ...
    @Query("SELECT a FROM Announcement a WHERE a.status = :status AND (a.expiresAt IS NULL OR a.expiresAt >= :now) ORDER BY a.pinned DESC, a.createdAt DESC")
    List<Announcement> findActiveByStatus(@Param("status") AnnouncementStatus status, @Param("now") Instant now);
    ...
    List<Announcement> findByStatusAndExpiresAtLessThan(AnnouncementStatus status, Instant now);
}
